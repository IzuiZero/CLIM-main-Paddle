...
class DeticCaption(nn.Layer):
    def __init__(self,
        ...
        if bce_bias is None:
            ...
            self.bce_bias = self.create_parameter(shape=[1], default_initializer=nn.initializer.Constant(bce_bias))
    ...
    def sample(rpn_results, batch_data_sample, **kwargs):
        ...
        return InstanceData(
            bboxes=image_boxes,
            labels=paddle.to_tensor([0] * len(image_boxes), place=image_boxes.place, dtype=paddle.int64),
            scores=paddle.to_tensor([1.0] * len(image_boxes), place=image_boxes.place),
            metainfo=batch_data_sample.metainfo
        )

    @paddle.no_grad()
    def get_caption_features(self, captions, place, clip_model):
        captions = [cap for b in captions for cap in b]
        tokens = CLIP.tokenize_dynamic(captions, truncate=True).to(place)
        caption_features = clip_model.text_encoder.encode_text(tokens, normalize=True).astype('float32')
        return caption_features

    def get_losses(self, region_embeddings, sampling_results, clip_model,
                   update_queue=True,
                   *args, **kwargs):
        region_embeddings = F.normalize(region_embeddings, axis=-1)
        place = region_embeddings.place
        ...
        all_captions, caption_image_ids = multi_apply(self._parse_captions_per_sample,
                                                      ...
                                                      region_image_ids)
        region_image_ids = paddle.concat(region_image_ids).to(place)
        caption_image_ids = paddle.concat(caption_image_ids).to(place)
        assert region_image_ids.shape[0] == region_embeddings.shape[0]

        clip_model.eval()
        clip_caption_features = self.get_caption_features(all_captions, place, clip_model)

        global_clip_caption_features = self.queues.get_queue('clip_caption_features')
        contrast_clip_caption_features = paddle.concat([clip_caption_features,
                                                        global_clip_caption_features[..., :-self.id_length]], axis=0)
        contrast_clip_caption_image_ids = paddle.concat([caption_image_ids,
                                                         global_clip_caption_features[..., -self.id_length:]], axis=0)

        # matrix 0
        image_id_match_matrix = region_image_ids[:, None] == contrast_clip_caption_image_ids[None]
        label_matrix_0 = image_id_match_matrix.astype('float32').sum(axis=-1).clip(max=1.0)
        similarity_matrix_0 = self.norm_temp * paddle.matmul(region_embeddings, contrast_clip_caption_features.T)
        ...
        pos_weights = paddle.ones_like(loss_caption) / num_pos_labels
        neg_weights = paddle.ones_like(loss_caption) * self.cap_neg_weight
        loss_caption *= paddle.where(label_matrix_0 > 0.0, pos_weights, neg_weights)

        loss_caption = loss_caption.sum(-1).mean()
        if update_queue:
            clip_caption_features_update = paddle.concat([clip_caption_features, caption_image_ids], axis=-1)
    ...
    def _parse_image_ids_per_sample(self, sampling_result):
        ...
        return paddle.stack(image_ids_tensor)

    def _image_id2tensor(self, img_id):
        template = -paddle.ones(self.id_length)
        if isinstance(img_id, (list, tuple)):
            num_imgs_ = len(img_id)
            template[:num_imgs_] = paddle.to_tensor(img_id)
        else:
            template[:] = img_id
        return template
    ...
    def _parse_captions_per_sample(self, pseudo_region_captions, pseudo_region_image_ids):
        ...
        for caps, image_id in zip(pseudo_region_captions, pseudo_region_image_ids):
            ...
            caption_image_ids.append(image_id[None].repeat(num_sampled_caps, 0))

        caption_image_ids = paddle.concat(caption_image_ids)
    ...
    def sample(self, rpn_results, batch_data_sample, **kwargs):
        ...
        if image_boxes.shape[0] > 1:
            ...
            for sampled_group in sampled_groups:
                ...
                x0y0 = sampled_boxes.reshape(-1, 2).min(axis=0).values
                x1y1 = sampled_boxes.reshape(-1, 2).max(axis=0).values
                added_boxes.append(paddle.concat([x0y0, x1y1]))
                captions.append([])
            ...
            added_boxes = paddle.stack(added_boxes)
            image_boxes = paddle.concat([image_boxes, added_boxes])
