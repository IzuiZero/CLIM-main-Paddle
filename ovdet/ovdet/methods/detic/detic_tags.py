...
class DeticTags(nn.Layer):
    def __init__(self,
        ...
        if tag_embeddings_path.endswith('npy'):
            tag_embeddings = paddle.to_tensor(np.load(tag_embeddings_path), dtype=paddle.float32)
        else:
            tag_embeddings = paddle.load(tag_embeddings_path).astype(paddle.float32)
        
        tag_embeddings = F.normalize(tag_embeddings, axis=-1)
    ...
    def sort_by_box_ids(proposals, box_ids):
        sorted_box_ids, sorted_order = paddle.argsort(box_ids)
        ...
        for box_id, image_box in enumerate(image_boxes):
            ...
            if len(box_id_proposals) == 0:
                ...
                if hasattr(proposals, "labels"):
                    data_dict.update(labels=paddle.to_tensor([0], dtype=paddle.int64,
                                                             place=image_boxes.place))
                if hasattr(proposals, "scores"):
                    data_dict.update(scores=paddle.to_tensor([1], place=image_boxes.place))
    ...
    def obtain_topk_proposal(self, proposals, box_ids):
        if hasattr(proposals, 'scores') and len(box_ids) > 0:
            unique_box_ids = paddle.unique(box_ids)
            topk_proposals = []
            topk_box_ids = []
            for box_id in unique_box_ids:
                ...
                _, topk_inds = paddle.topk(cand_proposals.scores, num)

                topk_proposals.append(cand_proposals[topk_inds])
                topk_box_ids.append(cand_box_ids[topk_inds])
            return InstanceData.cat(topk_proposals), paddle.concat(topk_box_ids)
        else:
    ...
    def assign_proposal2image_boxes(self, proposals, image_boxes):
        ...
        max_iofs, box_ids = paddle.max(iofs, axis=1)
    ...
    def get_losses(self, region_embeddings, sampling_results, *args, **kwargs):
        ...
        similarity_matrix_1 = self.norm_temp * paddle.matmul(region_embeddings, self.tag_embeddings.T)
        similarity_matrix_1 += self.bce_bias

        label_matrix_1 = paddle.zeros_like(similarity_matrix_1)
    ...
    def sample_on_topk(self, proposals, box_ids, image_boxes, metainfo):
        ...
        if image_boxes.shape[0] > 1:
            ...
            for sampled_group in sampled_groups:
                ...
                x0y0 = max_size_boxes_.reshape(-1, 2).min(axis=0).values
                x1y1 = max_size_boxes_.reshape(-1, 2).max(axis=0).values

                added_box = paddle.concat([x0y0, x1y1])
