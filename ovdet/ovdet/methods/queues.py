...
class Queues(nn.Layer):
    ...
    def _init_queues(self):
        ...
        for n in attr_names:
            self.register_buffer(n, -paddle.ones((1, self.emb_dim + self.id_length)), persistable=False)
    ...
    def dequeue_and_enqueue(self, queue_update):
        for k, feat in queue_update.items():
            ...
            queue_value.set_value(paddle.concat([feat, queue_value[:kept_length]]))
