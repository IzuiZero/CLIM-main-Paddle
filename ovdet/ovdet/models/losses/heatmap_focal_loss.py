...
def heatmap_focal_loss_with_pos_inds(
    ...
    pred = paddle.clip(
        F.sigmoid(pred), min=sigmoid_clamp, max=1 - sigmoid_clamp)

    neg_weights = paddle.pow(1 - targets, beta)

    pos_pred = pred[pos_inds]
    pos_loss = paddle.log(pos_pred) * paddle.pow(1 - pos_pred, gamma)
    neg_loss = paddle.log(1 - pred) * paddle.pow(pred, gamma) * neg_weights
    if ignore_high_fp > 0:
        not_high_fp = (pred < ignore_high_fp).astype(paddle.float32)
...
class HeatmapFocalLoss(paddle.nn.Layer):
    ...
    def forward(self,
        ...
        Args:
            pred (Tensor): The prediction. The shape is (N, num_classes).
            target (Tensor): The learning target of the prediction
                in gaussian distribution. The shape is (N, num_classes).
            pos_inds (Tensor): The positive sample index.
                Defaults to None.
            avg_factor (Union[int, float], optional): Average factor that is used to
