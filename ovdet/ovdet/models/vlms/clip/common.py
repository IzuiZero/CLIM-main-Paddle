...
class ResidualAttentionBlock(nn.Layer):
    ...
    def attention(self, x: paddle.Tensor, return_tokens: bool, attn_masks=None):
    ...
    def forward(self, x, return_tokens=False, cls_indices=None, attn_masks=None):
        ...
        if return_tokens:
            ...
            cls_tokens = x[cls_indices, paddle.arange(x.shape[1])]
            tokens = cls_tokens.unsqueeze(0) + tokens
...
class Transformer(nn.Layer):
    ...
    def forward(self, x: paddle.Tensor, return_tokens=False, cls_indices=None, attn_masks=None):
