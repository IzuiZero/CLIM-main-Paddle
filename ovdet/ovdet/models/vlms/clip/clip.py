...
from paddle.vision.transforms import Compose, Resize, CenterCrop, Normalize, InterpolationMode
...
if packaging.version.parse(paddle.__version__) < packaging.version.parse("2.0.0"):
    warnings.warn("PaddlePaddle version 2.0.0 or higher is recommended")
...
def load(name: str,
         device: Union[str, paddle.device.Device] = "cuda" if paddle.is_compiled_with_cuda() else "cpu",
         jit: bool = False,
    ...
    device : Union[str, paddle.device.Device]
    ...
    preprocess : Callable[[PIL.Image], paddle.Tensor]
        A paddle.vision transform that converts a PIL image into a tensor that the returned model can take as its input
    """
    if name in _MODELS:
    ...
    try:
        # loading JIT archive
        model = paddle.jit.load(model_path, map_location=device if jit else "cpu").eval()
        state_dict = None
    except RuntimeError:
        ...
        if jit:
            ...
            jit = False
        state_dict = paddle.load(model_path, map_location="cpu")
    ...
    device_holder = paddle.jit.trace(lambda: paddle.ones([], dtype='float32').to(paddle.device(device)), example_inputs=[])
    ...
    if str(device) == "cpu":
        float_holder = paddle.jit.trace(lambda: paddle.ones([], dtype='float32'), example_inputs=[])
