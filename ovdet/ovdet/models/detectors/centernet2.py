...
class CenterNet2(CascadeRCNN):
    ...
    def loss(self, batch_inputs: Tensor, batch_data_samples: SampleList) -> dict:
        ...
        if self.with_rpn:
            ...
            for data_sample in rpn_data_samples:
                data_sample.gt_instances.labels = paddle.zeros_like(data_sample.gt_instances.labels)

            rpn_losses, rpn_results_list = self.rpn_head.loss_and_predict(
                x, rpn_data_samples, proposal_cfg=proposal_cfg)

            # avoid get same name with roi_head loss
            keys = list(rpn_losses.keys())
            for key in keys:
    ...
    def predict(self, batch_inputs: Tensor, batch_data_samples: SampleList, rescale: bool = True) -> SampleList:
        ...
        Returns:
            SampleList: Return the detection results of the input images.
                The returns value is SampleList, which usually contain 'pred_instances'.
                And the `pred_instances` usually contains following keys.
