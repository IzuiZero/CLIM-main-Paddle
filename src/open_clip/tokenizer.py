...
# Ensure that tokenizers parallelism is disabled
import os
...
class SimpleTokenizer(object):
    def __init__(self, bpe_path: str = default_bpe(), special_tokens=None):
        ...
        self.vocab = Vocab.from_list(vocab)
    ...
    def encode(self, text):
        ...
        for token in re.findall(self.pat, text):
            ...
            bpe_tokens.extend([self.vocab[token] for token in self.bpe(token).split(' ')])
        return bpe_tokens

    def decode(self, tokens):
        text = ''.join([self.vocab.to_tokens(token) for token in tokens])
...
def tokenize(texts, context_length=77):
    ...
    sot_token = _tokenizer.vocab['<start_of_text>']
    eot_token = _tokenizer.vocab['<end_of_text>']
