...
def load_state_dict(checkpoint_path: str, map_location='cpu'):
    checkpoint = paddle.load(checkpoint_path, map_location=map_location)
...
def load_checkpoint(model, checkpoint_path, strict=True):
    ...
    incompatible_keys = model.set_state_dict(state_dict, use_structured_name=not strict)
...
def create_model(
    ...
    if pretrained and pretrained.lower() == 'openai':
        ...
        if precision in ("fp16", "bf16"):
            convert_weights_to_lp(model, dtype=paddle.bfloat16 if precision == 'bf16' else paddle.float16)
