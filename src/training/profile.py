...
import open_clip_paddle as open_clip  # Assuming you have a PaddlePaddle version of open_clip
import pandas as pd
...
def profile_fvcore(
    ...
    example_image_input = paddle.ones((batch_size,) + image_input_size, dtype=dtype)
    example_text_input = paddle.ones((batch_size,) + text_input_size, dtype='int64')
    fca = nn.FlopCountAnalysis(model, (example_image_input, example_text_input))
    aca = nn.ActivationCountAnalysis(model, (example_image_input, example_text_input))
    if detailed:
        fcs = nn.flop_count_str(fca)
        print(fcs)
...
def profile_fvcore_text(
    ...
    device = next(model.parameters()).device
    example_input = paddle.ones((batch_size,) + text_input_size, dtype='int64')
    fca = nn.FlopCountAnalysis(model, example_input)
    aca = nn.ActivationCountAnalysis(model, example_input)
    if detailed:
        fcs = nn.flop_count_str(fca)
        print(fcs)
    return fca.total(), aca.total()
...
def profile_fvcore_image(
    ...
    device = next(model.parameters()).device
    dtype = next(model.parameters()).dtype
    example_input = paddle.ones((batch_size,) + image_input_size, dtype=dtype)
    fca = nn.FlopCountAnalysis(model, example_input)
    aca = nn.ActivationCountAnalysis(model, example_input)
    if detailed:
        fcs = nn.flop_count_str(fca)
